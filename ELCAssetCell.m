...
- (void)setAssets:(NSArray *)assets andPreviousAssets:(NSMutableArray *)previousAssets
{
    ...
    for (int i = 0; i < [_rowAssets count]; ++i) {
        ...
        for (MyAsset *myAsset in previousAssets) {
            
            ALAssetRepresentation *assetRepresentation = [asset.asset defaultRepresentation];
            
            
            if ([myAsset.fileName isEqualToString:assetRepresentation.filename]) {
                
                [self autoSelectCell:i];
            }
        }
    }
}
